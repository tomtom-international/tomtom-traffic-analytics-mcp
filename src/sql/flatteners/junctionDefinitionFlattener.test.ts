...
describe("flattenJunctionDefinitions", () => {
  ...
  it("should handle junctions without junctionModel", () => {
    const result = flattenJunctionDefinitions([mockJunctionNoModel]);
    const rows = result.tables.get("junctions")!;
    expect(rows).toHaveLength(1);
  ...
  it("always produces junctions, approaches, and exits tables", () => {
    const result = flattenJunctionDefinitions([mockJunction]);
  ...
  it("handles empty input", () => {
    const result = flattenJunctionDefinitions([]);
    expect(result.tables.get("junctions")).toHaveLength(0);
    expect(result.tables.get("approaches")).toHaveLength(0);
    expect(result.tables.get("exits")).toHaveLength(0);
  });
  ...
  it("should handle multiple junctions", () => {
    const result = flattenJunctionDefinitions([mockJunction, mockJunctionNoModel]);
